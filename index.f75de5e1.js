...
const populations = Array.from(populationElements).map((element)=>{
    ...
    let numberString = "";
    for (const char of text)if (!isNaN(char) && char !== " ") numberString += char;
...
document.querySelector(".average-population").innerText = Number(average.toFixed(0)).toLocaleString("en-US");
